...
{
    public class GreekNseFoRepository : GreekRepository<TradeViewGreekNseFo>, IGreekNseFoRepository
    {
        ...
        private static ILog _log = LogService.GetLogger(typeof(GreekNseFoRepository));
        private readonly IConfiguration _configuration;
        private readonly IFileHelper _fileHelper;
        private readonly IMapper _mapper;

        private const string sourceFileConfigKey = "GreekFileNseFoSourceFilePath";
        private const string destinationFileConfigKey = "GreekFileNseFoFilePath";
        private const string BrokerId = "12562";
        private readonly string ClientCodeConst = "12562";
        private const string Source = "GREEK_NSE_FO";

        public GreekNseFoRepository(IGenericRepository<object> tradeViewGenericRepo, ITradeViewGenericRepository tradeViewRepo,
        ...
        {
            _log.Info($"GreekNseFoRepository: LoadTradeviewFromSource Starting Params: isDeltaLoadRequested: {isDeltaLoadRequested}");
            ...
            if(isDeltaLoadRequested)
            {
                ...
                _log.Info($"GreekNseFoRepository: LoadTradeviewFromSource Finished Params- isDeltaRequested:{isDeltaLoadRequested} Result-{lst?.Count}");
            }
            _log.Info($"GreekNseFoRepository: LoadTradeviewFromSource Finsihed Params:isDeltaLoadRequested | {isDeltaLoadRequested}");
        }
        
        public async Task LoadTradeviewFulDataFromSource()
        {
            _log.Info($"GreekNseFoRepository: LoadTradeviewFulDataFromSource Starting");
            var sourceFilePath = _configuration.GetValue<string>(sourceFileConfigKey).ToString();
            var destinationFilePath = _configuration.GetValue<string>(destinationFileConfigKey).ToString();
            var lst = GetDataFromSource(sourceFilePath, destinationFilePath, true);
            await LoadTradeViewRefTable(lst);
            _log.Info($"GreekNseFoRepository: LoadTradeviewFulDataFromSource Finished Result-{lst.Count}");
        }
